...
// Biến lưu trạng thái Thành phố đang chọn trong Dialog popup
String? selectedCityInDialog = user?.address?.city;
...
// DROPDOWNBUTTONFORMFIELD CHỌN CITY TRỰC TIẾP TỪ USER LIST CACHE
Consumer(
builder: (context, dialogRef, child) {
final userListAsync = dialogRef.watch(userViewModelProvider);

return userListAsync.when(
data: (users) {
// Trích xuất danh sách tất cả các thành phố không trùng lặp từ cache
final citiesList = users
...
hint: const Text('Select City Address', style: TextStyle(color: Colors.white38)),
...
onChanged: (value) {
// Sử dụng setDialogState để cập nhật UI Dropdown tức thì bên trong Dialog
setDialogState(() {
...
// Sử dụng Form Validation thực tế thay vì check tay đơn giản
...
address: selectedCityInDialog != null ? Address(city: selectedCityInDialog!) : null,
);
...
username: username,
email: email.isNotEmpty ? email : null,
address: selectedCityInDialog != null ? Address(city: selectedCityInDialog!) : user.address,
phone: user.phone,
